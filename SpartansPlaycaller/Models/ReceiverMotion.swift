...
enum ReceiverMotion: String, CaseIterable, Identifiable {
    ...
    /// Motion continues after the snap (route branch based on post-snap action).
    case after = "After"

    /// Motion immediately (pre-snap) to the opposite side.
    case go = "Go"
    ...
    /// - **after**: Receiver flips to opposite side during route execution.
    /// - **go**: Receiver flips to opposite side pre-snap and runs route from that side.
    ///
    ...
    func finalSide(originalSide: FieldSide) -> FieldSide {
        ...
        case .after, .go:
            // Both after and go flip the receiver to the opposite side
            switch originalSide {
            case .left:
                return .right
            case .right:
